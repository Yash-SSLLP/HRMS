...
(async () => {
  ...
  {
    Candidate.distinct = async () => ['Bright Placements'];
    // Typed in by HR, from an agency with no portal login (or a CV it emailed).
    const walkIn = new Candidate({ name: 'Mani', stage: 'Shortlisted', rounds: [{ label: 'Round 1' }] });
    walkIn.save = async () => walkIn;
    Candidate.findById = () => q(walkIn);
    const edit = (body) => run(rc.updateCandidate, { user: hr, params: { id: String(walkIn._id) }, body });
    const set = await edit({ name: 'Mani', consultancyName: '  bright   placements ', source: 'Application' });
    check("HR records it: filed under the agency's recorded spelling, source follows",
      [set.status, walkIn.source, walkIn.consultancy?.name], [200, 'Consultancy', 'Bright Placements']);
    check('...as a NAME only — never the portal link that hands an agency Round 1', walkIn.consultancy?.user, undefined);
    await edit({ name: 'Mani', stage: 'Interview' });
    check('an edit that never sends the box (the app) keeps it',
      [walkIn.source, walkIn.consultancy?.name, walkIn.stage], ['Consultancy', 'Bright Placements', 'Interview']);
    await edit({ consultancyName: '' });
    check('clearing the box puts them back to Portal', [walkIn.source, walkIn.consultancy?.name], ['Portal', undefined]);

    // The agency's own candidate: HR's form can neither rename nor unlink it.
    const theirs = new Candidate({ name: 'Asha', stage: 'Interview', source: 'Consultancy', consultancy: { user: agencyId, name: 'Bright Placements' }, rounds: [{ label: 'Round 1' }] });
    theirs.save = async () => theirs;
    Candidate.findById = () => q(theirs);
    await run(rc.updateCandidate, { user: hr, params: { id: String(theirs._id) }, body: { consultancyName: '', consultancy: { user: null }, source: 'Portal' } });
    check("an agency's own candidate keeps its agency, its link and its source",
      [String(theirs.consultancy?.user), theirs.consultancy?.name, theirs.source], [String(agencyId), 'Bright Placements', 'Consultancy']);

    let created = null;
    Candidate.create = async (doc) => { created = doc; return doc; };
    await run(rc.createCandidate, { user: hr, body: { name: 'New Walk In', consultancyName: 'Bright placements', consultancy: { user: agencyId } } });
    check('Add Candidate records it the same way — a name, no link',
      [created?.source, created?.consultancy], ['Consultancy', { name: 'Bright Placements' }]);
    await run(rc.createCandidate, { user: hr, body: { name: 'Direct', consultancyName: '' } });
    check('...and with the box blank adds them as Portal', [created?.source, created?.consultancy, 'consultancyName' in created], [undefined, undefined, false]);
    // An agency with a login that has not sent anybody yet is still "known".
    const userFind = User.find;
    User.find = () => q([{ firstName: 'Sunrise', lastName: 'Talent' }]);
    await run(rc.createCandidate, { user: hr, body: { name: 'Via Sunrise', consultancyName: 'SUNRISE talent' } });
    check("a typed name files under an agency login's spelling too", created?.consultancy, { name: 'Sunrise Talent' });
    User.find = userFind;
  }

  console.log('\n--- the consultancy dropdown ---');
  {
    const saved = { userFind: User.find, jobFind: Job.find, distinct: Candidate.distinct };
    const jobA = oid();
    let recordedFilter = null;
    User.find = () => q([
      { firstName: 'Bright', lastName: 'Placements', companies: [companyA] },
      { firstName: 'Far', lastName: 'Away', companies: [companyB] },
      { firstName: 'Every', lastName: 'Where', companies: [] },
    ]);
    Job.find = () => q([{ _id: jobA }]);
    Candidate.distinct = async (field, filter) => { recordedFilter = filter; return ['bright placements', 'Walk-in Agency']; };

    const all = await run(rc.listConsultancies, { user: { role: 'SuperAdmin' } });
    check('SuperAdmin: every agency, plus names HR recorded, once each',
      all.res.body?.consultancies, ['Bright Placements', 'Every Where', 'Far Away', 'Walk-in Agency']);
    const walled = await run(rc.listConsultancies, { user: { role: 'HRManager', scopeCompanyId: companyA } });
    check("company A's HR: not the agency that only serves company B",
      walled.res.body?.consultancies, ['Bright Placements', 'Every Where', 'Walk-in Agency']);
    check('...and recorded names are read inside the company wall',
      JSON.stringify(recordedFilter), JSON.stringify({ $or: [{ job: { $in: [String(jobA)] } }, { job: null }] }));
    check('the agency itself cannot fetch the list',
      externalRefusal({ method: 'GET', originalUrl: '/api/recruitment/consultancies', user: agency }), 'This account can only use the consultancy workspace.');

    Object.assign(User, { find: saved.userFind });
    Object.assign(Job, { find: saved.jobFind });
    Object.assign(Candidate, { distinct: saved.distinct });
  }
