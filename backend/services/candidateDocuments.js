...
// The portal's own letters, in the employee document vocabulary. Both are
// HR_ONLY_CATEGORIES in models/Document.js — an employee can never upload one,
// which is exactly right for a document the company issues.
const GENERATED_LETTERS = [
  { kind: 'offer', category: 'OfferLetter', label: 'Offer letter' },
  { kind: 'appointment', category: 'AppointmentLetter', label: 'Appointment letter' },
];
...
/**
 * Carry the generated offer + appointment letter PDFs onto the employee record.
 *
 * Kept apart from copyCandidateDocuments because the source and the verdict
 * differ. Those are files the candidate uploaded, and one HR verified during
 * hiring arrives already Verified. These the portal produced itself — and they
 * still arrive as **Submitted**, because releasing a letter to a candidate and
 * confirming the copy filed against the employee is the right one are two
 * different acts, and HR asked to be the one who does the second.
 *
 * Without this the letters only ever existed on the Candidate: the employee's
 * own record showed the scans they sent in and no sign of what the company
 * issued them.
 *
 * Deduped by content hash exactly like the uploads, so convertToEmployee and
 * the backfill can both run over the same person without producing a second
 * copy.
 *
 * @param {object} candidate - a Candidate document
 * @param {import('mongoose').Types.ObjectId} profileId - the EmployeeProfile
 ...
 * @returns {Promise<{copied: number, skipped: number, failed: number, details: string[]}>}
 */
async function copyCandidateLetters(candidate, profileId, actorId, opts = {}) {
  const result = { copied: 0, skipped: 0, failed: 0, details: [] };
  const wanted = GENERATED_LETTERS.filter((l) => candidate[l.kind]?.letterPath);
  if (!wanted.length) return result;

  const existing = await Document.find({ employee: profileId }).select('sha256').lean();
  const seen = new Set(existing.map((d) => d.sha256).filter(Boolean));

  for (const { kind, category, label } of wanted) {
    const letter = candidate[kind];
    const fileName = letter.letterName || `${label.replace(/ /g, '-')}.pdf`;
    try {
      // eslint-disable-next-line no-await-in-loop
      const buffer = await storage.readBuffer(letter.letterPath);
      const sha = sha256Of(buffer);
      if (seen.has(sha)) {
        result.skipped += 1;
        result.details.push(`${label}: already on the employee`);
      ...
      // Re-saved under the employee rather than sharing the candidate's storage
      // path, for the same reason the uploads are: one file behind two rows
      // means deleting either takes the other's bytes with it.
      // eslint-disable-next-line no-await-in-loop
      const saved = await storage.saveBuffer({
        buffer, ownerType: 'employee', ownerId: profileId, originalName: fileName,
      });
      ...
      await Document.create({
        ...
        isPii: PII_CATEGORIES.includes(category),
        uploadedBy: actorId,
        note: `Generated by the portal${letter.generatedAt ? ` on ${istDate(letter.generatedAt)}` : ''}`,
        status: 'Submitted',
        // Marks this as a letter the PORTAL produced, not one somebody scanned
        // and uploaded. Issuing a fresh appointment letter from the employee
        // record looks for this so it can offer to supersede the old one rather
        // than leaving two contracts on file with no way to tell which is in
        // force. See controllers/employeeLetterController.js.
        generatedBy: 'candidate-letter',
      });
      seen.add(saved.sha256);
      result.copied += 1;
      result.details.push(`${label} -> ${category}`);
    } catch (err) {
      // A letter whose bytes are gone must not cost the other one, nor the
      // conversion that called this.
      result.failed += 1;
      result.details.push(`${label}: ${err.message}`);
    }
  }
...
module.exports = {
  copyCandidateDocuments, copyCandidateLetters, CANDIDATE_DOC_CATEGORY, MIME_BY_EXT,
};
