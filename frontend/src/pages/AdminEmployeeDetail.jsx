            ...
            {docs.map((d) => (
              <li key={d._id} className="py-2 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  {/* `note` says where the file came from ("Generated by the
                      portal on …", "Submitted during hiring as …"), which is
                      what separates an issued letter from an upload. */}
                  {(d.note || d.reviewNote) && (
                    <div className="text-xs text-gray-400 truncate">{[d.note, d.reviewNote].filter(Boolean).join(' · ')}</div>
                  )}
                </div>
