 ...
 * and the Status cell says which: "Your approval" (it has cleared its named
 * approvers, or never had any) or "With <name>" (still climbing). Deciding the
 * second is an OVERRIDE — it voids the remaining steps — so it asks first
 ...
 * not here — see the note on the page shell at the bottom of this file.
 * Unconfigured employees come straight to HR in this queue.
 */
...
/*
 * WHERE THE APPROVAL SETUP WENT. Who signs off each employee's corrections used
 * to be a second tab here. It now lives on the Permissions page, with every
 * other access decision in the portal — naming who may decide somebody else's
 * request is not attendance administration, it is access, and an access review
 * should be one screen rather than a tour of the modules. The grant is
 * unchanged (`regularizationHierarchy.manage`, or the older `hierarchy.manage`),
 * and so is the component: see
 * components/permissions/RegularizationApprovalSetup.jsx.
 *
 * One tab left, so no segmented strip: there is nothing to switch between.
 */
